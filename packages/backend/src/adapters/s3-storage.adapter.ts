...
export class S3StorageAdapter implements StoragePort {
  ...
  private async getClient(orgId: string | null): Promise<{ client: S3Client, bucket: string }> {
    const config = await this.settingsService.getSetting(orgId, 'storage.s3.config', true);
    if (!config || !config.accessKeyId || !config.secretAccessKey) {
      throw new Error(`S3 configuration missing for organization ${orgId || 'system'}`);
    }
  ...
  async list(orgId: string | null, prefix: string): Promise<string[]> {
    try {
        ...
        return (response.Contents || []).map(obj => obj.Key || '').filter(k => k);
    } catch (error) {
