...
export class FixTrunkDataTables1766332000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP VIEW IF EXISTS ps_registrations, ps_identifies`,
    );
    await queryRunner.query(
      `CREATE VIEW ps_registrations AS SELECT * FROM ps_registrations_data`,
    );
    await queryRunner.query(
      `CREATE VIEW ps_identifies AS SELECT * FROM ps_identifies_data`,
    );
  }
