...
export class AddStunToDatabase1766334000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "ps_endpoints_data" ADD COLUMN IF NOT EXISTS "stunaddr" TEXT`,
    );

    // 2. Update existing users to use Google STUN
    await queryRunner.query(
      `UPDATE "ps_endpoints_data" SET "stunaddr" = 'stun.l.google.com:19302' WHERE webrtc = 'yes'`,
    );
    ...
    const selectClause = columnsResult
      .map((col: any) => {
